type bound =
 ...
 let compare_bound ( b : bound ) (b': bound) : int =
  ...
  let sample_interval (b1 : bound) (b2 : bound) : Real.t  = 
    match b1, b2 with 
    |Finite b1_val , Finite b2_val -> Real.div ( Real.add b1_val b2_val) (Real.of_int 2 )
    ...
    let is_good_covering (l : interval list) : bool =
      ...
      let sample_outside  (l : interval list ) : Real.t option =
        let rec loop_open c l =
          ...
          | Open (a, _) :: _ when compare_bound  (Finite c)  a <= 0  -> Some (sample_interval (Finite c) a)
          ...
          module S = Set.Make (struct 
            ...
            let rec check_pairs eliminate lst =
              ...
              | i1 :: i2 :: rest -> 
                  match i1, i2 with
                  | Open (_, b), Open (_, d) -> 
                      let eliminate = if compare_bound d b <= 0 then S.add i2 eliminate else eliminate in
                      check_pairs eliminate (i2 :: rest)
                  | Open (_, b), Exact a ->  
                      let eliminate = if compare_bound (Finite a) b < 0 then S.add i2 eliminate else eliminate in
                      check_pairs eliminate (i2 :: rest)
