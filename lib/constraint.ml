...
let mk_pply_array poly_specs = Array.map mk_poly poly_specs
...
(*#######"#"#"#"##"##"#*************************************************************************)
  ...
  let required_coefficients ( s : Real.t array ) ( p : Polynomes.t list  ) : Z_poly.t list  =
    let rec loop l acc : Z_poly.t list   = 
      match l  with 
      |[] -> acc 
      |q :: l ->
          loop l (Polynomes.required_coefficient s q ) @ acc   
      in loop p [] 
      ...
      let p3  = mk_pply_array [| [ ([ 1 ], 1); ([ 0 ], 2) ]; [ ([ 0 ], -4) ] |] 
